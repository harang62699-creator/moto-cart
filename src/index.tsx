...
async function openForm(formType) {
  ...
  setTimeout(() => { if (window.initFieldImgAttach) initFieldImgAttach(document.getElementById('form-content')); }, 200);
}
...
.dp-header-tbl {
  width:100%; border-collapse:collapse;
  border:1px solid #888; margin-bottom:6px;
}
.dp-header-lbl-cell {
  border:1px solid #888; padding:4px 8px;
  background:rgba(79,142,247,.08); text-align:center;
...
.dp-header-inp {
  ...
  font-size:.9rem; padding:3px 0; outline:none;
  font-family:inherit; color:#111;
}
@media print {
  .dp-wrap { padding:0; background:#fff; }
  .dp-tbl th, .dp-tbl td { border:1px solid #333 !important; padding:2px 4px !important; }
  .dp-sec-th { background:#d6e4f7 !important; -webkit-print-color-adjust:exact; print-color-adjust:exact; }
  .dp-sub-th, .dp-th { background:#eef3fa !important; -webkit-print-color-adjust:exact; print-color-adjust:exact; }
  .dp-lbl, .dp-lbl2 { background:#f5f8ff !important; -webkit-print-color-adjust:exact; print-color-adjust:exact; }
  .dp-header-lbl-cell { background:rgba(79,142,247,.08) !important; -webkit-print-color-adjust:exact; print-color-adjust:exact; }
}
</style>
...
<table class="dp-tbl">
  ...
  <tr>
    <th class="dp-th">설비, 장비명</th><th class="dp-th">제작사</th><th class="dp-th">모델명</th>
...
<table class="dp-tbl">
  ...
  <tr>
    <th class="dp-th" colspan="2">감지변수</th>
  ...
  <tr>
    ...
    <th class="dp-th">제조국 / 비고</th>
  </tr>
  \${['점화장치','연료공급장치','배출가스 전환장치','배출가스 재순환장치','연료증발가스 방지장치','브로바이가스 환원장치','2차공기 분사장치'].map((item,ri)=>\`<tr>
    <td class="dp-lbl" colspan="2">\${item}</td>
    <td><input class="dp-inp" data-field="dp_8_11_pn_\${ri}" type="text" value="\${E(v('dp_8_11_pn_\${ri}'))}"></td>
    <td><input class="dp-inp" data-field="dp_8_11_maker_\${ri}" type="text" value="\${E(v('dp_8_11_maker_\${ri}'))}"></td>
    <td><input class="dp-inp" data-field="dp_8_11_country_\${ri}" type="text" value="\${E(v('dp_8_11_country_\${ri}'))}"></td>
  </tr>\`).join('')}
...
<table class="dp-tbl">
  ...
  \${['차명','자동차 형식','차체형상','승차인원','사용연료','배기량(cc)','연소방식','실린더 수','최대출력(ps/rpm)','최대토크(kg-m/rpm)','보어*스트로크(mm)','압축비','실린더 보어 중심간의 거리(mm)','실린더 블록 형상','실린더 배열','실린더 헤드 방식','공기 흡입 방식'].map((row,ri)=>\`<tr>
    <td class="dp-lbl" colspan="2">\${row}</td>
  ...
  \${['배출가스 인증번호','자동차 명칭','자동차 형식','원동기 형식','차종','사용연료','배출가스 자가진단 장치의 작동법','배출가스 허용기준','연소싸이클','연료공급방식','촉매전환장치 형태','입자상물질 포집장치 형태','2차 공기 분사 유무','배출가스 재순환장치 유무'].map((row,ri)=>\`<tr>
    <td class="dp-lbl" colspan="2">\${row}</td>
    <td><input class="dp-inp" data-field="dp_10_3_base_\${ri}" type="text" value="\${E(v('dp_10_3_base_\${ri}'))}"></td>
    <td><input class="dp-inp" data-field="dp_10_3_same_\${ri}" type="text" value="\${E(v('dp_10_3_same_\${ri}'))}"></td>
  </tr>\`).join('')}
</table>
...
// ================================================================
(function initGlobalImgAttach() {
  // ── 전역 모달 DOM 생성 ──────────────────────────────────────────
  const overlay = document.createElement('div');
  overlay.id = 'img-attach-overlay';
  ...
  modal.style.cssText = [
    'background:#fff;border-radius:12px;padding:20px 24px;width:480px;max-width:95vw',
    'box-shadow:0 8px 40px rgba(0,0,0,.35);position:relative;max-height:85vh;overflow-y:auto'
  ].join(';');

  modal.innerHTML =
    '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;">' +
      '<div style="font-size:13pt;font-weight:700;color:#1e3a5f;">' +
        '<i class="fas fa-image" style="margin-right:6px;color:#4e90d8;"></i>이미지 첨부' +
      '</div>' +
      '<button id="img-attach-close" style="background:none;border:none;font-size:18px;cursor:pointer;color:#666;padding:2px 6px;">&times;</button>' +
    '</div>' +
    '<div id="img-attach-field-label" style="font-size:8.5pt;color:#888;margin-bottom:10px;"></div>' +
    '<div id="img-attach-drop" style="border:2px dashed #b0c4de;border-radius:8px;padding:18px 12px;text-align:center;cursor:pointer;background:#f7faff;transition:all .2s;margin-bottom:12px;">' +
      '<input type="file" id="img-attach-file" accept="image/*" multiple style="display:none;">' +
      '<div id="img-attach-hint" style="color:#7a9cc0;font-size:9pt;">' +
        '<i class="fas fa-cloud-upload-alt" style="font-size:20px;display:block;margin-bottom:6px;"></i>' +
        '클릭하거나 이미지를 여기에 드래그하세요' +
      '</div>' +
    '</div>' +
    '<div id="img-attach-list" style="display:flex;flex-wrap:wrap;gap:8px;margin-bottom:8px;"></div>' +
    '<div style="display:flex;gap:8px;justify-content:flex-end;">' +
      '<button id="img-attach-cancel" style="padding:7px 18px;border:1px solid #ccc;border-radius:6px;background:#fff;cursor:pointer;font-size:9pt;">취소</button>' +
      '<button id="img-attach-ok" style="padding:7px 18px;background:#2563eb;color:#fff;border:none;border-radius:6px;cursor:pointer;font-size:9pt;font-weight:600;">확인</button>' +
    '</div>';
  ...
  style.textContent = [
    ...
    '  background:none;border:1px solid #b0c4de;border-radius:4px;',
    '  cursor:pointer;color:#4e90d8;font-size:10px;flex-shrink:0;',
    '  vertical-align:middle;padding:0;transition:all .15s;',
    '}',
    '.field-img-btn:hover { background:#eef3fa;border-color:#4e90d8; }',
    '.field-img-btn.has-img { background:#dbeafe;border-color:#2563eb;color:#2563eb; }',
    '.field-img-thumbs { display:flex;flex-wrap:wrap;gap:4px;margin-top:3px; }',
    '.field-img-thumbs img { width:48px;height:48px;object-fit:cover;border-radius:4px;border:1px solid #b0c4de;cursor:pointer; }',
    '.img-attach-item { position:relative;width:90px;height:90px; }',
    '.img-attach-item img { width:100%;height:100%;object-fit:cover;border-radius:6px;border:1px solid #ccc; }',
    '.img-attach-item-del { position:absolute;top:-6px;right:-6px;width:18px;height:18px;',
    ...
    '#img-attach-drop.drag-over { border-color:#2563eb;background:#dbeafe; }',
    '@media print {',
    '  .field-img-btn { display:none !important; }',
  ...
  // ── 상태 ─────────────────────────────────────────────────────────
  let currentField = null;   // 현재 대상 data-field 명
  let currentHidden = null;  // hidden input
  let currentThumbEl = null; // 썸네일 컨테이너
  let images = [];           // base64 dataURL[]

  // ── 유틸 ─────────────────────────────────────────────────────────
  function renderModalList() {
    const list = document.getElementById('img-attach-list');
    list.innerHTML = '';
    images.forEach(function(src, i) {
      const item = document.createElement('div');
      item.className = 'img-attach-item';
      const img = document.createElement('img'); img.src = src;
      const del = document.createElement('button');
      del.className = 'img-attach-item-del';
      del.textContent = '\xd7';
      del.onclick = function(e) { e.stopPropagation(); images.splice(i,1); renderModalList(); };
  ...
  function renderThumbs() {
    if (!currentThumbEl) return;
    currentThumbEl.innerHTML = '';
    images.forEach(function(src) {
      const img = document.createElement('img');
      img.src = src;
      img.title = '클릭하여 크게 보기';
      img.onclick = function() { window.open(src,'_blank'); };
      currentThumbEl.appendChild(img);
    });
  }

  function syncHidden() {
    if (currentHidden) currentHidden.value = JSON.stringify(images);
    // 버튼 상태 업데이트
    if (currentField) {
      document.querySelectorAll('.field-img-btn[data-img-field="'+currentField+'"]')
        .forEach(function(btn) {
          btn.classList.toggle('has-img', images.length > 0);
          btn.title = images.length > 0 ? '이미지 '+images.length+'장 첨부됨 (클릭하여 편집)' : '이미지 첨부';
        });
    }
  }
  ...
  // ── 모달 열기 ────────────────────────────────────────────────────
  function openModal(field, hiddenEl, thumbEl, labelText) {
    currentField  = field;
    currentHidden = hiddenEl;
    currentThumbEl = thumbEl;
    try { images = JSON.parse(hiddenEl.value || '[]'); } catch { images = []; }
    document.getElementById('img-attach-field-label').textContent = labelText || field;
    renderModalList();
  ...
  function closeModal(apply) {
    if (apply) { renderThumbs(); syncHidden(); }
    overlay.style.display = 'none';
  }

  // ── 모달 이벤트 ──────────────────────────────────────────────────
  document.getElementById('img-attach-close').onclick  = function() { closeModal(false); };
  document.getElementById('img-attach-cancel').onclick = function() { closeModal(false); };
  document.getElementById('img-attach-ok').onclick     = function() { closeModal(true); };
  overlay.addEventListener('click', function(e) { if (e.target===overlay) closeModal(false); });

  const dropZone = document.getElementById('img-attach-drop');
  const fileInput = document.getElementById('img-attach-file');
  ...
  // ── 실제 필드에 버튼 삽입 (공개 함수) ───────────────────────────
  window.initFieldImgAttach = function(container) {
    if (!container) return;
    // dp-inp, dp-ta, en-inp, summary-inp 등 data-field 를 가진 모든 입력 요소
    container.querySelectorAll('[data-field]').forEach(function(el) {
      // hidden input, checkbox는 제외
      ...
      const imgField = field + '_imgs';

      // hidden input 확인 또는 생성
      let hidden = container.querySelector('input[type="hidden"][data-field="'+imgField+'"]');
      if (!hidden) {
        hidden = document.createElement('input');
        hidden.type = 'hidden';
        hidden.dataset.field = imgField;
        hidden.value = '[]';
      ...
      // 썸네일 컨테이너 (버튼 다음에 삽입)
      let thumbEl = el.parentNode.querySelector('.field-img-thumbs[data-for="'+field+'"]');
      if (!thumbEl) {
        thumbEl = document.createElement('div');
        thumbEl.className = 'field-img-thumbs';
        thumbEl.dataset.for = field;
      }

      // 📎 버튼 생성
      const btn = document.createElement('button');
      btn.type = 'button';
      btn.className = 'field-img-btn';
      btn.dataset.imgField = field;
      btn.title = '이미지 첨부';
      btn.innerHTML = '<i class="fas fa-paperclip"></i>';

      // 기존 저장값 복원
      let savedImgs = [];
      try { savedImgs = JSON.parse(hidden.value || '[]'); } catch {}
      if (savedImgs.length > 0) {
        btn.classList.add('has-img');
        btn.title = '이미지 ' + savedImgs.length + '장 첨부됨 (클릭하여 편집)';
        savedImgs.forEach(function(src) {
          const img = document.createElement('img');
      ...
      btn.onclick = function(e) {
        ...
        // 라벨 텍스트: 부모 th 또는 td.dp-lbl 텍스트
        let labelText = field;
        const row = el.closest('tr');
        if (row) {
          const lbl = row.querySelector('td.dp-lbl,td.dp-lbl2,th');
          if (lbl) labelText = lbl.textContent.trim().slice(0,40);
        }
        openModal(field, hidden, thumbEl, labelText);
      };

      // 삽입 위치: el 다음에 버튼 → 그 다음에 썸네일
      const parent = el.parentNode;
      const next = el.nextSibling;
      parent.insertBefore(btn, next);
      parent.insertBefore(thumbEl, btn.nextSibling);
    });
  };

  // ── 폼 렌더 후 자동 감지 (MutationObserver) ──────────────────────
  const fc = document.getElementById('form-content');
  if (fc) {
    const mo = new MutationObserver(function(mutations) {
      mutations.forEach(function(m) {
        m.addedNodes.forEach(function(n) {
          if (n.nodeType===1) setTimeout(function(){ window.initFieldImgAttach(n); }, 100);
        });
      });
    });
    mo.observe(fc, { childList: true, subtree: false });
  }
})();
